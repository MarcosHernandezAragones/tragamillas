...
<script>
    ...
    async function cambiar_pass(){
        ...
        if (pass == confirm){
            ...
            await fetch('<?php echo RUTA_URL?>/usuarios/cambiar_pass/', {
            method: "POST",
            body: data,
            })
            .then((resp) => resp.json())
    ...
    async function rellenarTabla(usuarios){
        ...
        for (let i = 0; i < usuarios.length; i++) {
            ...
            if (usuarios[i].rol != 0) {
                td_acciones.innerHTML ="<button type=\"button\" class=\"btn me-2 btn-outline-primary btn-block\" data-bs-toggle=\"modal\" data-bs-target=\"#Modal_Borrar\" onclick=\"cambiar_modal('ver');usuarioActual("+usuarios[i].id_user+",1)\"><i class=\"bi bi-eye\"></i></button><button type=\"button\" class=\"btn me-2 btn-block editUruario\"  data-bs-toggle=\"modal\" data-bs-target=\"#exampleModal\" onclick=\"usuarioActual( "+usuarios[i].id_user+",0)\"><i class=\"bi bi-pencil-square\"></i></button><button type=\"button\" class=\"btn me-2 btn-outline-danger btn-block\" data-bs-toggle=\"modal\" data-bs-target=\"#Modal_Borrar\" onclick=\"cambiar_modal('borrar');usuarioActual( "+usuarios[i].id_user+",1)\"><i class=\"bi bi-person-x-fill\"></i></button><button type=\"button\" class=\"btn me-2 btn-outline-secondary btn-block\" data-bs-toggle=\"modal\" data-bs-target=\"#Modal_Pass\" onclick=\"cambiarPass("+usuarios[i].id_user+",'"+usuarios[i].email+"')\"><i class=\"bi bi-shield-lock-fill\"></i></button>"
            } else {
                td_acciones.innerHTML ="<button type=\"button\" class=\"btn btn-outline-secondary btn-block\" data-bs-toggle=\"modal\" data-bs-target=\"#Modal_Pass\" onclick=\"cambiarPass("+usuarios[i].id_user+",'"+usuarios[i].email+"')\"><i class=\"bi bi-shield-lock-fill\"></i></button> ";
            }
    ...
    var element_per_paige=6//ffs: or any other number  //ffs: futuras versiones poder cambiar el num elementos
